...
class Network(nn.Module):
    ...
    @staticmethod
    def grad_orientation_loss_local_weighted_ez(
        pred,
        mono,
        k=64,
        s=32,
        ...
        """
        pred : [B,1,H,W] (student depth)
        mono : [B,1,H,W] (teacher depth)
        출력  : gradient orientation distillation loss (scalar)
        """

        # 0) mono 유효 마스크 (3x3 이웃까지 모두 유효한 픽셀만 살리기)
        with torch.no_grad():
            m = (mono > 0).float()  # [B,1,H,W]
            # invalid = maxpool(1-m) == 1 이면 주변에 하나라도 0이 있는 것
            invalid = nnf.max_pool2d(1.0 - m, kernel_size=3, stride=1, padding=1)
            valid = (invalid == 0).float()  # [B,1,H,W]

        # 1) Sobel로 기울기/크기
        x = pred
        t = mono.to(x.device, x.dtype).detach()

        sobel_x = _SOBEL_X.to(x.device, x.dtype)
        sobel_y = _SOBEL_Y.to(x.device, x.dtype)

        def sobel_grad(z):
            z = nnf.pad(z, (1, 1, 1, 1), mode=pad_mode)
            gx = nnf.conv2d(z, sobel_x)
            gy = nnf.conv2d(z, sobel_y)
            return gx, gy

        gx_p, gy_p = sobel_grad(x)
        gx_t, gy_t = sobel_grad(t)

        magp = torch.sqrt(gx_p * gx_p + gy_p * gy_p + eps)
        magt = torch.sqrt(gx_t * gx_t + gy_t * gy_t + eps)

        # base: 방향 불일치 (scale-invariant)
        cos = ((gx_p * gx_t + gy_p * gy_t) / (magp * magt + eps)).clamp(-1.0, 1.0)
        base = 1.0 - cos  # [B,1,H,W]

        # 2) 로컬 통계로 가중치 생성 (mono의 grad magnitude 기준)
        mt = magt.detach()
        p = k // 2
        mtp = nnf.pad(mt, (p, p, p, p), mode="reflect")

        mu_loc = nnf.avg_pool2d(mtp, k, stride=s)
        m2_loc = nnf.avg_pool2d(mtp * mtp, k, stride=s)
        var_loc = (m2_loc - mu_loc * mu_loc).clamp_min(0.0)
        std_loc = torch.sqrt(var_loc + eps) + std_floor

        if s > 1:
            mu_loc = nnf.interpolate(mu_loc, size=mt.shape[-2:], mode="bilinear", align_corners=False)
            std_loc = nnf.interpolate(std_loc, size=mt.shape[-2:], mode="bilinear", align_corners=False)

        # 로컬 표준화된 대비로 시그모이드 게이팅
        z_loc = (mt - mu_loc) / (beta * std_loc + eps)
        w = w0 + (1.0 - w0) * torch.sigmoid(z_loc)

        # --- 여기서부터 mono==0 제외 ---
        w = w * valid  # [B,1,H,W]

        # 유효 픽셀 수
        valid_sum = valid.sum(dim=(2, 3), keepdim=True)  # [B,1,1,1]

        # 유효 영역 평균 1이 되도록 배치별로 정규화
        w_mean_over_valid = (w.sum(dim=(2, 3), keepdim=True) / (valid_sum + eps))
        w = (w / (w_mean_over_valid + eps)).detach()
    ...
    def forward_train(self, mini_batch_data: dict):
        ...
        # 🔥 orientation-based distillation (teacher gradient 방향 따라가기)
        ori_loss = self.grad_orientation_loss_local_weighted_ez(
            pred,
            teacher,
            k=64,
            s=32,
        ...
        # distill_loss 변수는 그대로 쓰되, 내부는 orientation loss로 대체
        distill_loss = ori_loss
        ...
        loss = l1_loss + lambda_distill * distill_loss + lambda_llr * llr_loss
        ...
        monitors = {
            ...
            "distill_loss":distill_loss.detach(), # = ori_loss
