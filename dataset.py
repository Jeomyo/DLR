...
class Vidar(torch.utils.data.Dataset):
    ...
    def load_panoptic(self, cam_info, depth_mask_np):
        ...
        pp_sem = sem_map.astype(np.int16)[None, ...]            # (1, H, W)
...
if __name__ == '__main__':
    ...
    def visualize_panoptic_sem(pp_sem_1hw, valid_mask_1hw=None):
        """
        pp_sem_1hw: (1, H, W) int16 (category_id)
        valid_mask_1hw: (1, H, W) uint8 (0/1) or None
            - None이면 전체 panoptic를 색칠
            - 주어지면 그 mask 위치만 남기고 나머지 0으로
        """
        sem = pp_sem_1hw.squeeze().astype(np.int32)          # (H, W)

        if sem.max() == 0:
            # 전부 0인 경우 (panoptic 없음)
            return np.zeros((sem.shape[0], sem.shape[1], 3), dtype=np.uint8)

        # category id를 [0,255]로 스케일링해서 colormap
        sem_norm = (sem.astype(np.float32) / sem.max()) * 255.0
        sem_norm = sem_norm.astype(np.uint8)  # (H, W)

        sem_color = cv2.applyColorMap(sem_norm, cv2.COLORMAP_JET)  # (H, W, 3)

        # valid_mask가 주어졌으면 그 부분만 남기기
        if valid_mask_1hw is not None:
            vm = valid_mask_1hw.squeeze().astype(bool)  # (H, W)
            sem_color[~vm] = 0
