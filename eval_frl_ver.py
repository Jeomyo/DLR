...
class VidarEval:
    def __init__(self, pkl_path, data_root):
        ...
        self.panoptic_root = '/home/jhkim/jeomyo/pseudo_panoptic'
    ...
    def load_panoptic(self, cam_info, depth_mask_np):
        ...
        # 그냥 (1, H, W)로 감싸서 리턴
        pp_inst = inst_map.astype(np.int32)[None, ...]
        return pp_inst
        
    def set_model(self, model_path, use_flr=False):
        self.model_path = model_path
        checkpoint = torch.load(model_path, map_location=self.device)

        self.net = Network(use_flr=use_flr).to(self.device)
    ...
    def __getitem__(self, index):
        ...
        pp_inst_np = self.load_panoptic(cam_info, depth_mask_np)    # (1, H, W)

        return img_np, depth_gt, img_path, pp_inst_np
    ...
    def eval(self, model_path=None, use_flr=False, max_samples=None, save_vis=False, vis_dir='eval_vis'):
        """Evaluation 수행"""
        if model_path is not None:
            self.set_model(model_path, use_flr=use_flr)
        ...
        for idx in tqdm(range(n_samples), desc='Evaluating'):
            img_np, depth_gt, img_path, pp_inst_np = self[idx]
            ...
            label_mask = ((depth_gt > 0) & (depth_gt < conf.max_depth)).astype(np.float32)[None, None, ...]  # (1, 1, H, W)
            pp_inst = pp_inst_np[None, ...]
            # ============================
            # Forward pass
            # ============================
            mini_batch = {
            ...
            with torch.no_grad():
                out = self.net.forward_eval(mini_batch, use_flr=use_flr)  # out: dict 또는 tensor
...
def main():
    ...
    parser.add_argument('-d', '--dataset', type=str, default=conf.default_dataset,
                       choices=['nuscenes'],
                       help='Dataset name')
    parser.add_argument('--use_flr', action='store_true',
                       help='Use FLR module (requires panoptic)')
